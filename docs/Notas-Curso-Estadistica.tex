...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{knitr}\OperatorTok{::}\KeywordTok{kable}\NormalTok{(}\KeywordTok{data.frame}\NormalTok{(Mé}\DataTypeTok{todo =} \KeywordTok{c}\NormalTok{(}\StringTok{"Jacknife"}\NormalTok{, }\StringTok{"Bootstrap Normal"}\NormalTok{, }
...
\section{Ejemplo del viajero}\label{ejemplo-del-viajero}}
...
\includegraphics{Notas-Curso-Estadistica_files/figure-latex/unnamed-chunk-126-1.pdf}
...
\includegraphics{Notas-Curso-Estadistica_files/figure-latex/unnamed-chunk-126-2.pdf}
...
\section{El algoritmo de Metropolis-Hasting}\label{el-algoritmo-de-metropolis-hasting}}
...
una verosimilitud de \(f(z, N | \theta)=\theta^{z}(1-\theta)^{(N-z)}\).
...
\item
  Probabilidad de aceptación \$ p\_\{\text {moverse }\}\$
  \begin{align*}
   p_{\text {moverse }}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{ggplot}\NormalTok{(df) }\OperatorTok{+}\StringTok{ }\KeywordTok{geom_histogram}\NormalTok{(}\KeywordTok{aes}\NormalTok{(P, }\DataTypeTok{y =}\NormalTok{ ..density..), }
